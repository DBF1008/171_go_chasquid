...
B<chasquid-util> [I<options>] user-list I<domain> [--receive_only] [--prefix=I<prefix>]
...
=item B<user-list> I<domain> [--receive_only] [--prefix=I<prefix>]

List the users in the domain, sorted by name. Receive-only users (see
I<--receive_only> in B<user-add>) are marked as such in the output.

If I<--receive_only> is given, only receive-only users are listed. If
I<--prefix> is given, only users whose name starts with the given prefix are
listed.
